...
-- ON DELETE CASCADE: 기준 테이블(customer)의 데이터가 삭제되면 참조 테이블(orderlist)의 데이터도 삭제
...
SELECT * FROM customer INNER JOIN orderlist ON customer.id = orderlist.customer_id AND quantity >= 5;
...
c.name, o.product_name, o.quantity 
