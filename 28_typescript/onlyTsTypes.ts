...
let drink2: [string, string, number] = ["cola", "콜라", 1200];
console.log(drink2);
drink2[0] = "사이다";
console.log(drink2);

// readonly옵션: 타입과 순서를 완벽히 고정
// 추후 수정 불가능
...
const student1: Student = { name: "gildong", isPassed: true };
...
interface 야구부 extends Student {
  ...
  weight: number;
  height: number;
  readonly backNumber?: number; // 없어도 되는 값 ?로 처리
  [grade: number]: Score;
}
...
otani["1"] = "A+";
otani.position = "투수";
// otani.backNumber = 11; // readonly는 수정 불가
...
const sum: Calc = (num1: number, num2: number) => {
  return num1 + num2; // number 리턴턴
};

interface Toy {
  name: string;
  start(): void; // 함수 중 리턴타입이 없을 경우 void
}
interface Car {
  name: string;
  color: string;
