...
console.log(createHashPw("1234")); // 1ARV~
console.log(createHashPw("1234")); // 1ARV~
// 조금만 변해도 전혀 다른 해시값이 나옴
console.log(createHashPw("1234.")); // mGcAiWg~
...
function saltAndHashPw(pw) {
  ...
  //   const hash = crypto.pbkdf2Sync(pw, salt, iterations, keylen, algorithm); // Buffer 객체를 리턴하는 중 (1001111010)
  const hash = crypto
...
// salt: '94Hq72qOxpDzxMlnBjFxmA==',
// hash: 'BmNuo9EoLhoxrN0L31n2~
console.log("pdkdf2Sync >> ", saltAndHashPw("1234"));

// salt: 'pOoXgBeXPdpI9978IQUefw==',
// hash: 'ESzODq/YByUum~
console.log("pdkdf2Sync >> ", saltAndHashPw("1234"));
...
function checkPw(inputPw, savedSalt, savedHash) {
  ...
  const keylen = 64; // 길이 saltAndHashPw와 같아야 함.
  const algorithm = "sha512"; // 알고리즘 saltAndHashPw와 같아야 함.
...
function encrypt(text) {
  ...
  // 2. 실제 데이터 암호화
  //   const encrypted = cipher.update(평문, 입력인코딩, 출력인코딩)
...
console.log(encrypt(originalMessage)); // 6pPaP64SAd8aEwqr9zpdvw==
console.log(encrypt(originalMessage)); // 6pPaP64SAd8aEwqr9zpdvw==
